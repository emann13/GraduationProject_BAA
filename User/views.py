...
def patient_data(request, patientID):
    patient_id = patientID
    patient = get_object_or_404(Patient, PatientID=patient_id)
    patient_xrays = X_ray.objects.filter(PatientID=patient_id)
    xrays = []
    for xray in patient_xrays:
        xray_data = {
            # 'Img': xray.Img.url,
            'Bone_age': xray.Bone_age,
...
def get_patient_data(request):
    patients = Patient.objects.filter(DoctorID=request.user.UserID)
    patient_data_list = []
    for patient in patients:
        patient_xrays = X_ray.objects.filter(PatientID=patient.PatientID)
        xrays = []
        for xray in patient_xrays:
            xray_data = {
                # 'Img': xray.Img.url,
...
def add_patient(request):
    ...
    # print(photos)
    # for photo in photos:
    #   image_path = default_storage.save('images/' + photo.name, photo)
...
def edit_profile(request):
    ...
    profile= {
        ...
        'email' : request.user.email,
        'PhoneNumber'  : request.user.PhoneNumber ,
        # 'hospital_name' : request.user.DisplayName ,  
        'UserID' : request.user.UserID,
        'Img' : Img,
     }
